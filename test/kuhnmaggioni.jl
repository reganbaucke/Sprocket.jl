...
include("../src/KuhnMaggioni/KuhnMaggioni.jl")
...
function division_test()
   function build_problem()
      ...
      division = Sprocket.Point(Dict(xi => 3))

      return (problem, division)
   end
   (my_prob, division) = build_problem()
   KuhnMaggioni.cartesian_product(division, my_prob.domain)
end

function division_test_two()
   ...
   function build_problem()
      ...
      division = Sprocket.Point(Dict(xi => 3, lam => 2))
...
function division_test_three()
   function build_problem()
      ...
      #set the domain of the random variable
      Sprocket.set_domain(problem,(Sprocket.Point(Dict(xi =>[0.0,0.0])), Sprocket.Point(Dict(xi => [1.0,1.0]))))
      ...
      division = Sprocket.Point(Dict(xi => [2,3] ))
...
function test_one()
   ...
   states = Sprocket.solve(KuhnMaggioni.Algorithm(), my_prob, my_criteria)
